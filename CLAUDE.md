...
- **Send Invite is structured in two phases.** Phase 1 saves the collaborator document (source of truth) — the writes (`saveProjectMeta` + `addCollaborator`) are raced against a 10s timeout. On timeout, the handler reads the doc back via `getCollaborators` (5s budget) before declaring failure: `setDoc` requires a server ACK that can be delayed by network conditions even when the write itself succeeded, so a verification read prevents false "timed out" errors. If verified, "Invite sent!" is shown and the input cleared **immediately** — Phase 2 must never gate that. Only if the doc is genuinely absent does the handler show "Invite timed out, please try again". Phase 2 is a separate non-blocking notification step with a 3s timeout that skips silently on failure (today it's a no-op; the invitee receives the pending invite through their own collectionGroup `onSnapshot` listener on `/collaborators` when they sign in). The handler is wrapped in `try/finally` so `setShareLoading(false)` always runs, and `loadCollaborators()` is fired without awaiting.
...
- **Remove collaborator** uses an optimistic UI update (the row disappears immediately), races the delete against a 10s timeout, restores the previous list and surfaces an error on failure, and refreshes via background `loadCollaborators()` without awaiting.
