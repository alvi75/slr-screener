...
- **Team Progress** — visible to all collaborators (not just owner). `openTeamDashboard` uses `overrideProjectId || urlProjectSlug || projectId` as the effective project ID. Owner ID determined from multiple sources: `meta.ownerId`, then `collab.ownerId`, then `collab.invitedBy`, then `projectOwnerId` state, then `null` (never current `userId`). Accepted collaborators with missing `userId` are backfilled when the collaborator visits their own dashboard. Deduplication via `addedIds` Set prevents any user appearing twice. Current user always first with "(You)" label. Auto-refreshes every 15 seconds. Only aggregate counts shown for bias protection.
