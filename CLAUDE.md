...
- **Google sign-in** — one-click OAuth, bypasses email verification. Uses `signInWithPopup` on desktop, `signInWithRedirect` on mobile (detected via viewport width < 768 or user agent). `getRedirectResult` awaited before `onAuthStateChanged` listener is attached — prevents race condition where auth state fires `user=null` before redirect result resolves, which caused flash-redirect-to-login on mobile.
