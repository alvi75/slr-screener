...
- **Collaborator dedup**: `getCollaborators` always returns lowercased emails and at most one row per email — duplicates from older case-divergent docs are deleted server-side as a fire-and-forget cleanup. `addCollaborator` does an existence check first and throws `{ code: 'already-invited' }` if a doc exists, so a re-invite never overwrites an accepted collaborator.
...
- **Collaborators are always deduped by lowercased email** before render. `getCollaborators` does the dedup at fetch time (and cleans up case-divergent docs server-side); `dedupeCollabsByEmail` in App.js is a defensive second pass applied wherever `setCollaborators` runs (initial load, `loadCollaborators`, `openTeamDashboard`). Status-rank tiebreak: `accepted` > `pending`; then most recent `invitedAt`.
- **Duplicate invites are blocked at three layers**: (1) `inviteInFlightRef` prevents a rapid double-click from firing two writes, (2) the client checks the local list with case-insensitive `email.toLowerCase()` comparison, (3) `addCollaborator` reads the doc first and throws `{ code: 'already-invited' }` if it exists. The handler maps that error code to the exact message "This person has already been invited".
- **Send Invite — single awaited write, everything else fire-and-forget.** The handler optimistically inserts a `{ email, role, status: 'pending' }` row into `collaborators` immediately, then `await`s exactly one Firestore write — `fsAddCollaborator` (the source of truth). `fsSaveProjectMeta` is fire-and-forget (it's idempotent, only matters for collectionGroup discovery, and shouldn't gate the success message). `loadCollaborators` runs in the background. The notification step is fire-and-forget via `Promise.resolve().then(...).catch(() => {})` — never awaited. On the awaited write succeeding, "Invite sent!" is shown and the input cleared. On rejection, the optimistic row is reverted and the real error is surfaced. **No timeout wrappers** — Firestore completes naturally; an artificial timeout produces false "timed out" errors when the server ACK is just slow.
- **Do NOT call `fsSaveNotification` from the invite path.** Notifications reach the invitee through their own collectionGroup `onSnapshot` listener on `/collaborators` — the inviter never has the invitee's userId at invite time (they may not have an account yet), so any direct write to `users/{inviteeId}/notifications` from this path would either block or miss.
- **Remove collaborator — single awaited delete.** Optimistically filter the row out of `collaborators`, then `await fsRemoveCollaborator` (no timeout). Refresh via background `loadCollaborators()`. Revert + show real error on rejection.
